...
Once the data is in Delta Lake, Amazon SageMaker can be used to remove the heavy lifting from each step of the ML process to make it easier to develop high-quality models. [Amazon SageMaker ](https://aws.amazon.com/sagemaker/)is a fully managed ML service that gives data scientists and developers the speed and ease to build, train, and deploy models directly into a production-ready hosted environment. An Integrated JupyterLab environment opens data sources for exploration and analysis without managing servers.
...
straightforward and users are able to quickly and seamlessly switch between
single node (local mode) and distributed (cluster mode) environments.
...
After reading Delta Lake data in SageMaker Studio’s managed JupyterLab environment, users can access purpose-built tools to perform all machine learning (ML) development steps, from preparing data to building, training, and deploying your ML models.

Users can also benefit from the breadth and depth of SageMaker’s features built using both open-source as well as proprietary technologies for simplifying the complex and iterative process of Machine Learning. To get started with Amazon SageMaker and it’s end-to-end Machine Learning experience, click [here](https://aws.amazon.com/sagemaker/?nc=sn&loc=1).
