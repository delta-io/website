...
Delta Lake stores all metadata in a separate transaction log. Query engines can access all the metadata with a single read and quickly know which data to skip. This is much faster.
...
ORC tables are easier to corrupt because they do not support transactions. If you’re writing a large among of data to an existing ORC table and your cluster dies, you will be left with a failed partial write. A corrupted table can break a lot of downstream data systems and requires a manual hotfix. This will likely be time-consuming and expensive, and definitely not fun to do.
...
Delta Lake provides automatic maintenance operations like automatic compactions and optimized writes to help you deal with the small file problem. These operations will compact your smaller files into larger, more performant ones.
...
1. It assumes you know all of the queries you are going to run on the data, which can lead to expensive repartitioning when your queries change.
...
When writing new data to an existing table, you usually want to make sure that the new data follow the same schema as your existing dataset. Otherwise you will end up with mismatched columns that can break downstream systems. This process is known as schema enforcement, which helps maintain dataset consistency—part of the "C" in ACID.

Regular ORC tables don’t support schema enforcement by default. This means that it’s possible to corrupt your table by appending data with a mismatched schema:
...
- need ACID transactions for production-grade reliability
