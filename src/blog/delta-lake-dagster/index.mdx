...
Read more about these Delta Lake features in the [Delta Lake vs Data Lake tutorial](https://delta.io/blog/delta-lake-vs-data-lake/).
...
- Install your project as an editable package by running `pip install -e .` in your project root
...
We will walk through our pipeline step by step and then compile it into one single Python script that will go into the <code>[assets.py](assets.py)</code> script that we've just created.
...
Yes, you can use the Delta Lake and Dagster integration with other query engines. Take a look at [the Delta Lake documentation](https://delta-io.github.io/delta-rs/integrations/delta-lake-dagster/#using-delta-lake-and-dagster-with-polars) for instructions on how to run queries with Polars.

You can also check out the [Delta Lake in Python tutorial](#link-when-live) for more examples of using Delta Lake with different query engines.
