...
Checkpoints record the state of the data processing pipeline at a given point in time. This enables Delta Lake to make sure that data is processed exactly once, even in the event of failures, and enables the system to continue processing from the correct position if something goes wrong.
