...
show you how to use the new [`deltaray`](https://github.com/delta-incubator/deltaray) library to easily integrate the two technologies.
...
[Ray](https://docs.ray.io/en/latest/), on the other hand, is a popular distributed computing framework that enables you to build and run
...
[deltalake](https://github.com/delta-io/delta-rs) is the Python interface for the delta-rs project, a Native Delta Lake implementation in Rust.
...
create dummy Delta Tables because `deltaray` hasn’t yet implemented Ray’s write APIs. This is an active area of
development where the community can help enhance Delta Lake and Ray's integration.
...
Then you’ll create an initial Delta Table and update it twice so that there will be multiple versions of the same table.
