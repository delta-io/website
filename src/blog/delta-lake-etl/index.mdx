...
This article uses the term “ETL” to refer to any data workload that involves extracting, transforming and loading data, _in whichever order_ you might perform those operations. Most data projects are not perfectly linear anyway, see [ETL vs ELT](#internal-link) below. You may need to go back and forth between transforming and loading a few times. Delta Lake makes this easy and reliable.
...
You could extract the data from the sink and transform it \_before \_storing it, for example like this:
