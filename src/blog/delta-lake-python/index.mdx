...
Delta Lake stores your data in Parquet files and adds [a transaction log](#link-to-delta-architecture-blog) to give you powerful features like:
...
Many Python developers are using Python because it is lightweight and does not depend on the JVM. If that's you, then you will want to use the `delta-rs` implementation. If you are familiar with Spark and don't mind the Java dependencies, then [using Delta Lake with PySpark](#link-to-pyspark-blog) might be a good option.
