...
This article describes best practices when using <DeltaLake/>.
...
(that is, a large number of distinct values), then use `Z-ORDER BY`. <DeltaLake/> automatically lays out
...
You can compact a table using the [OPTIMIZE](optimizations/file-mgmt.md#delta-optimize) command.
...
<Info title="AWS Note" level="info">
...
If you want to change the table schema, you can replace the whole table atomically. For example:
    ...
    <CBCreateTable
        ...
        Databricks does not recommend that you use [Spark caching](optimizations/delta-cache.md#delta-and-rdd-cache-comparison) for the following reasons:
