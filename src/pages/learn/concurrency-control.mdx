...
- Readers continue to see a consistent snapshot view of the table that the <ApacheSpark/> job started with, even when a table is modified during a job.

## Optimistic concurrency control

<DeltaLake /> uses <Link href="https://en.wikipedia.org/wiki/Optimistic_concurrency_control">
  optimistic concurrency control
</Link> to provide transactional guarantees between writes. Under this
...
The following table describes which pairs of write operations can conflict. Compaction refers to `file compaction operation` written with the option `dataChange` set to `false`.

|                                | **INSERT**      | **UPDATE, DELETE, MERGE INTO** | **COMPACTION**  |
| ------------------------------ | --------------- | ------------------------------ | --------------- |
| **INSERT**                     | Cannot conflict | Can conflict                   | Cannot conflict |
| **UPDATE, DELETE, MERGE INTO** |                 | Can conflict                   | Can conflict    |
| **COMPACTION**                 |                 |                                | Can conflict    |
