---
title: Delta Lake on Apache Spark - Batch operations
width: full
---
...
<DeltaLake /> supports all the options provided by <ApacheSpark /> for
performing batch reads and writes on tables.

For many <DeltaLake /> operations on tables, you have to enable integration with <ApacheSpark />
DataSourceV2 and Catalog APIs (since 3.0) by setting [configurations when you create a
new `SparkSession`]().

## Create a Table

Delta Lake supports creating two types of tables — tables defined in the metastore and tables defined by path.
To work with metastore-defined tables, you have to enable integration with <ApacheSpark />
DataSourceV2 and Catalog APIs (since 3.0) by setting [configurations when you create a
new `SparkSession`]().
...
- **SQL DDL commands**: You can use standard SQL DDL commands supported in Apache Spark (for example, CREATE TABLE and REPLACE TABLE) to create Delta tables.
  ...
  Spark SQL also supports creating a table at a path, without creating an entry in the Hive metastore.
...
- **DataFrameWriter API**: If you want to simultaneously create a table and insert data into it from Spark DataFrames or Datasets,
  you can use the Spark `DataFrameWriter` ([Scala or Java](https://spark.apache.org/docs/latest/api/scala/org/apache/spark/sql/DataFrameWriter.html) and
...
To determine whether a table contains a specific partition, check if there are non-zero records in that partition. For example:
...
/>
If the partition exists, `true` is returned.

### Control Data Location

For tables defined in the metastore, you can optionally specify the path as the table location. Tables created with a specified location are considered unmanaged by the metastore. Unlike a managed table, where no path is specified, an unmanaged table’s files are not deleted when you DROP the table.
...
- Since Delta Lake 1.1.0, `MERGE` operations support generated columns when you set `spark.databricks.delta.schema.autoMerge.enabled` to `true`.

Since Delta Lake 1.2.0, it may be able to automaticallly generate partition filters from data filters if a partition column is generated from data columns by one of the following expressions:
...
You can use SQL [`EXPLAIN`](https://spark.apache.org/docs/latest/sql-ref-syntax-qry-explain.html) and check the provided plan to see whether Delta Lake automatically generates any partition filters.

### Use special characters in column names

By default, special characters such as spaces and any of the characters `,;{}()\n\t=` are not supported in table column names. To include these special characters in a table's column name, enable [column mapping](https://docs.delta.io/latest/versioning.html#-column-mapping).
...
<Delta /> 2.0 and above supports _dynamic_ partition overwrite mode for
partitioned tables.
...
You can configure dynamic partition overwrite mode by setting the Spark session configuration `spark.sql.sources.partitionOverwriteMode` to `dynamic`. You can also enable this by setting the `DataFrameWriter` option `partitionOverwriteMode` to `dynamic`. If present, the query-specific option overrides the mode defined in the session configuration. The default for `partitionOverwriteMode` is `static`.
...
<Info title="Important" level="warning">
Validate that the data being written with dynamic partition overwrite touches only the expected partitions. A single row in the incorrect partition can lead to unintentionally overwritting an entire partition. We strongly recommend using `replaceWhere` to explicitly specify which data to overwrite.

If a partition has been accidentally overwritten, you can use the [restore operation](https://docs.delta.io/latest/delta-utility.html) to undo the change.

</Info>

For <Delta /> support for updating tables, see the [update operations](https://docs.delta.io/latest/delta-update.html).

<a id="max-records-per-file"></a>

### Limit rows written in a file

You can use the SQL session configuration `spark.sql.files.maxRecordsPerFile` to specify the maximum number of records to write to a single file for a <DeltaLake /> table. Specifying a value of zero or a negative value represents no limit.
...
To rename columns without rewriting any of the columns' existing data, you must enable column mapping for the table. See [enable column mapping](https://docs.delta.io/latest/versioning.html#-column-mapping).
...
<Info title="Note" level="info">
  This feature is available in <DeltaLake /> 2.0 and above.
</Info>

To drop columns as a metadata-only operation without rewriting any data files, you must enable column mapping for the table. See [enable column mapping](https://docs.delta.io/latest/versioning.html#-column-mapping).
To drop a column:
...
Spark SQL will pass all of the current [SQL session configurations](http://spark.apache.org/docs/latest/configuration.html#runtime-sql-configuration) to <DeltaLake />, and <DeltaLake /> will use them to access Hadoop FileSystem APIs. For example, `SET a.b.c=x.y.z` will tell <DeltaLake /> to pass the value `x.y.z` as a Hadoop configuration `a.b.c`, and <DeltaLake /> will use it to access Hadoop FileSystem APIs.
...
You can find the details of the Hadoop file system configurations for your storage in the [storage page]().
