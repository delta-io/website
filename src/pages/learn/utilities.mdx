...
- The ability to [time travel](https://delta.io/docs/spark/batch#deltatimetravel) back to a version older than the retention period is lost after running `vacuum`.
- `vacuum` deletes only data files, not log files. Log files are deleted automatically and asynchronously after checkpoint operations. The default retention period of log files is 30 days, configurable through the `delta.logRetentionDuration` property which you set with the `ALTER TABLE SET TBLPROPERTIES` SQL method. See [table properties](https://delta.io/docs/spark/batch#table-properties).
