...
For many Delta Lake operations on tables, you enable integration with Apache s DataSourceV2 and Catalog APIs (since 3.0) by setting configurations when you create a new `SparkSession`. See [\_](#sql-support).
...
To work with metastore-defined tables, you must enable integration with Apache Spark DataSourceV2 and Catalog APIs by setting configurations when you create a new `SparkSession`. See [\_](#sql-support).
...
For Delta Lake support for updating tables, see [\_](delta-update.md).
