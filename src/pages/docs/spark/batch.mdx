---
title: Delta Lake on Apache Spark - Batch reads and writes
menu: docs
---
...
The data files backing a Delta table are _never_ deleted automatically; data files are deleted only when you run [VACUUM](utilities#delta-vacuum). `VACUUM` _does not_ delete Delta log files; log files are automatically cleaned up after checkpoints are written.
...
Provides information about schema, partitioning, table size, and so on. For details, see [delta details](utilities#delta-detail).

### `DESCRIBE HISTORY`

Provides provenance information, including the operation, user, and so on, and operation metrics for each write to a table. Table history is retained for 30 days. For details, see [delta history](utilities#delta-history).
...
You can find the details of the Hadoop file system configurations for your storage in [delta storage](/delta/delta-storage-oss.md).
