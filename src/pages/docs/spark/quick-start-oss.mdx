...
1. Run interactively: Start the Spark shell (Scala or Python) with <DeltaLake />
   and run the code snippets interactively in the shell.

2. Run as a project: Set up a Maven or SBT project (Scala or Java) with <DeltaLake />, copy the code snippets into a source file, and run the project.
...
1. Install the PySpark version that is [compatible](releases-oss.md) with the <DeltaLake /> version by running the following:
...
2. Run PySpark with the <DeltaLake /> package and additional configurations:
